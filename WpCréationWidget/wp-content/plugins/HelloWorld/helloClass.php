...
class HelloClass{  
    ...
    {//méthode déclenchée à l'activation du plug-in     
        global $wpdb;     
        $wpdb->query("CREATE TABLE IF NOT EXISTS {$wpdb->prefix}test (id INT AUTO_INCREMENT PRIMARY KEY, nom VARCHAR(255) NOT NULL, commentaire VARCHAR(255) NOT NULL);"); 
    }
    ...
    {  
        if (isset($_POST['helloworld_comm']) && !empty($_POST['helloworld_comm'] || isset($_POST['helloworld_nom']) && !empty($_POST['helloworld_nom'])))   
        {
            ...
            $row = $wpdb->get_row("SELECT * FROM {$wpdb->prefix}test WHERE commentaire = '$comm' AND nom = '$nom'");
            if (is_null($row)) {    
                $wpdb->insert("{$wpdb->prefix}test", array('commentaire' => $comm, 'nom' => $nom));   
            }  
        } 
    } 
} 