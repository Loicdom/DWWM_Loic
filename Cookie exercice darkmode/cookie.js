...
function changeCouleur(maValeur) {
    ...
    mesParagraphes = document.getElementsByClassName("txt");
    titreh1 = document.getElementsByClassName("titre1")[0];
    titreh1.setAttribute("class", "titre1 " + nouvClass);
    for (let i = 0; i < mesParagraphes.length; i++) {
        mesParagraphes[i].setAttribute("class", "txt " + nouvClass);
    }
    document.body.setAttribute("class", nouvClass);
    createCookie("color",monRange.value,7);
}